...
| Q-08 | DBOS + serverless-Postgres (Neon) operational fit. **Partly answered:** DBOS's `DrizzleDataSource` needs an interactive TCP pool → **neon-http unsupported; use Neon's TCP endpoint (likely direct/unpooled).** DBOS queue polling default is **`minPollingIntervalMs`=1000ms** (per-queue, +jitter/backoff), plus a system-DB heartbeat layer → **a running DBOS process keeps Neon warm, so it effectively never autosuspends** (Neon idle window ~5min ≫ 1s polls). Raising the poll interval past the autosuspend threshold would defeat job-pickup latency, so **fast pickup and scale-to-zero are mutually exclusive.** **Cost sub-question answered from public sources (no spike needed):** any recurring query defeats Neon autosuspend (Neon's own docs/GH call this out for health checks, poolers, cron/monitoring polling — general, not DBOS-specific; Neon idle default 5 min). ⇒ **Accept Neon stays warm.** A warm *minimum* compute is 0.25 CU ≈ **~$1.33/mo** on the Scale plan (~$0.222/CU-hr), autoscaling up only during import bursts — **cheaper than the ~$5–8/mo always-on Railway Postgres** and elastic, so Neon stays the right host; we just don't rely on scale-to-zero. **Remaining (cheap, at integration/staging):** confirm DBOS works on Neon's pooled vs direct endpoint, and system-DB placement on the same Neon instance end-to-end. | open | |
