...
>
> Verified against installed `@mastra/core@1.63.2`:
>
> 1. **Delegation mechanism — `deliberate` `createTool`, NOT native `agents:{}`.** Native Mastra
>    sub-agent delegation returns only the sub-agent's *text*, losing `DeliberationResult`
>    (`artifacts` can't render deterministically) and the offline `reasoner.run` spy seam. So: the
>    responder is a Mastra supervisor `Agent`; the reasoner stays its **own** Mastra `Agent`; the
>    supervisor reaches it via a hand-rolled Mastra `createTool` named `deliberate` whose `execute`
>    calls `reasoner.run(ctx, db)` and returns the real `DeliberationResult`. Verify the current
>    `createTool` signature against context7 `/mastra-ai/mastra` before wiring.
> 2. **A task turn is TWO generations, not one.** structured-output + a tool-call in one DeepSeek
>    generation is the two-pass gotcha. Keep the proven split: (gen-1) a tool-enabled decide pass
>    that may call `deliberate`; (gen-2) a tool-free `structuredOutput:{bubbles}` render pass over the
>    `DeliberationResult` — this is today's `MastraResponder.render`. A **social** turn is one render
>    generation; the reasoner is never invoked. Wherever the text below says "one supervisor run",
>    read "one turn, ≤2 generations".
> 3. **P1-1 — empty-plan render.** Today's render forces `bubbles.min(1)`. Short-circuit: when
>    `communicate`/`ask`/`artifacts` are all empty, return `[]` **before** calling the agent (so AC-4
>    holds on the live path, matching the reasoner's existing empty-plan branch).
> 4. **P1-2 — tapback grounding moves to the supervisor.** The reshape drops `intents`/`address`, so
>    the old react-vs-text rule (response-agent.ts:150) has no inputs. The social branch owns the
>    react/tapback decision using `triggerExternalId` (react only with a real target; else degrade to
>    text — AC-7).
> 5. **P1-4 — social turns pass `confirmTasks: []` explicitly** (do not inherit `loadTurn`'s list), or
>    a social bypass could spuriously confirm/pop tasks the reasoner never advanced (AC-5).
> 6. Keep the `Reasoner`/`Responder` interfaces + `Scripted*` doubles as the injected seam; Mastra
>    lives only in the `Mastra*` impls. Tests exercise the social/task decision + `reasoner.run` spy
>    offline through the scripted doubles.
