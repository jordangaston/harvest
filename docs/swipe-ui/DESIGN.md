...
| Q-08 | Does the public recipe card on the deck include ingredients/steps and all badge fields, or is a `GET /v1/recipes/:id` detail fetch needed for the DetailSheet? | **confirmed — gap** | The deck's `PublicRecipeCard` carries only `id, title, cost_per_serving_cents, total_minutes, difficulty_band` + the score `breakdown`. It does **not** carry `nrf_score` (nutrition), `meal_prep_fit`, `equipment`, diet/allergen `compat`, or `ingredients`/`steps` — all of which the SwipeCard + DetailSheet render. **Wiring the live deck (WI-3) is blocked until either the deck card DTO is enriched with the accent-badge fields, or the card hydrates ingredients/steps + the accent via a `GET /v1/recipes/:id` detail-fetch-on-open.** The core badges (time/cost/difficulty) + the "why" line (from `breakdown`) render from the deck card as-is. |
