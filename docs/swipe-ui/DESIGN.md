...
| Q-08 | Does the public recipe card on the deck include ingredients/steps and all badge fields, or is a `GET /v1/recipes/:id` detail fetch needed for the DetailSheet? | **resolved** | Both paths shipped. The deck card was **enriched** (WI-5) with `nrf_score`, `meal_prep_fit`, `equipment`, plus the recipe's `allergens` + compatible `diets` (the client derives the `compat` chips). Ingredients/steps stay off the card and are **hydrated lazily** by the DetailSheet via `GET /v1/recipes/:id?fields=ingredients,steps` — the detail endpoint now supports field selection so a caller fetches exactly what it needs. |
