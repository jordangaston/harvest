...
| 2026-08-05 | WI-05 | PR #6 review: monolithic parse-provider; Claude fallback; `toPublicRecipe` before use; missing caption-first | Decomposed into memoized `@DBOS.step`s; **caption-first tiered parse** (try the free caption before spending ASR/vision); removed the speculative Anthropic escalation; deleted the unused public projection; offline-gated the credential-free fetchers so tests never hit the network. |
