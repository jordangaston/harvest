...
> **Style supersedes:** implement to `server/CLAUDE.md` — **no DI container/composition root** (the `db`
> singleton is imported directly; classes use `static create()` factories; tests construct classes with a
> stub provider/repo directly). **Domain models are Zod schemas** parsed at the repo boundary
> (`UserSchema.parse(row)`). Where this spec below says "composition root" / "container override", read that
> as the equivalent in the singleton+factory style. DBOS is not in the scaffold anymore (it lands in WI-03).
