...
| **C5** nutrition (**parsed-only**) | ✅ Done | 8 label-core columns + `nutrition_source` enum (0008), string-nullable Zod. **Parsed path only:** when a source publishes schema.org `NutritionInformation` (`mapRecipe`, `fetch/website.ts`) → store with `nutrition_source='parsed'`; else null. Proof: `website-nutrition.test.ts`, `import.test.ts` (stub publishes none → null), `demos/backend-c3-c5-parsed.txt`. **Parsed coverage on 20 real halfbakedharvest.com recipes: 20/20** (`demos/coverage-parsed.txt`). |
| **C5a** food catalog | ⛔ **Punted** (founder, Rev 3) | Computed-from-USDA nutrition **removed** — generic matching couldn't reach usable accuracy (**13/20** recipes cleared the ≥0.6 floor, median ~61%). Deleted `FoodCatalog`, the `FoodMatcher`/Dice matcher, `NutritionService.compute`, the coverage floor, `server/seed/foods.json`, `build-foods-seed.ts`, `toGrams`, and their unit tests. Schema unchanged (no migration). See DESIGN.md Rev-3 banner. |
...
- **Computed nutrition (C5a) was punted (founder, Rev 3).** A real-USDA rebuild of the catalog (1,647 foods
  from SR Legacy + Foundation) was built and measured first, but generic-name → generic-USDA matching cleared
  the ≥0.6 floor on only **13/20** real recipes (median ~61%) — not good enough to show as authoritative macros.
  The **parsed path covers 20/20** of the same recipes (WP-Recipe-Maker sites publish `NutritionInformation` on
  every recipe), so C5 keeps parsed-only. All compute infra removed; schema unchanged. Evidence:
  `demos/coverage-parsed.txt` (parsed 20/20) vs the punted computed run (13/20, recorded in POSTMORTEM D9).
