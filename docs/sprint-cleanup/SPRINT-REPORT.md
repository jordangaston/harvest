...
- ~~**Matcher accuracy** … `foods.json` macros are USDA-sourced and hand-curated~~ — **Done** (follow-up): rebuilt `server/seed/foods.json` from the **real USDA FoodData Central** exports (SR Legacy base + Foundation overlay) via `scripts/build-foods-seed.ts` — **1,647 foods, ~525 KB**, per-100g label-core taken verbatim, curated head-noun names + aliases + a curated staples table, count/cup portions backfilled. Raw source files (210 MB / 6.7 MB) are **not committed**. **Real coverage** (live, 20 halfbakedharvest.com recipes through the actual `WebsiteFetcher` parse → catalog): **13/20 now compute nutrition** (clear the ≥0.6 floor) vs ~0/20 on the old 45-food seed; **median coverage 61%, mean 63%**. Evidence: `demos/coverage-hbh.txt` (+ `demos/hbh-urls.txt`). The 7 that stay null are lattes/cocktails and phyllo/garnish-heavy dishes whose ingredients (spirits, "for serving" garnishes, sub-recipe references) genuinely can't be quantified. Residual: matching is still lossy on the long tail; the floor + `nutrition.unmatched_ingredient` logs keep it honest.
