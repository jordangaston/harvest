...
const seasonings = JSON.parse(readFileSync('seed/seasonings.json', 'utf8')) as { label: string; names: string[] }[];
...
// Reset prior seasoning assignments (on existing foods too) so re-runs don't dangle.
await client.execute("update fdc_foods set base_ingredient_id = null where base_ingredient_id like 'seasoning-%'");
await client.execute('delete from fdc_foods where fdc_id >= 90000000'); // triggers keep the FTS mirrors in sync
await client.execute("delete from taste_ingredients where section = 'Seasoning'");
...
console.log(`seeded ${seasonings.length} seasoning bases · ${foods} synthetic foods · adopted ${adopted} existing base-less foods`);
process.exit(0);
