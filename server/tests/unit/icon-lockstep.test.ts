...
/**
 * Architect must-fix: the two icon maps must stay in lockstep. An ingredient icon
 * lives in three places — a keyword row in `server/src/parse/icons.ts` (name → key),
 * the app `ICON` map in `components/recime/recipes.ts` (key → asset), and the PNG in
 * `assets/ingredients/`. A key added to one but not the others renders a blank (or a
 * silent Harvest-H fallback), so this test fails the build if they drift.
 */
