...
const FACTORIES: Record<string, (ctx: TurnContext, db: Database) => ChefTool> = {
  household__add_members: AddMembersTool.create,
  recipes__import: ImportRecipeTool.create,
  facts__read: ReadFactsTool.create,
  facts__catalog: FactTypesTool.create,
  facts__update: UpdateFactsTool.create,
  tasks__update: UpdateTasksTool.create,
  mealplan__generate: GenerateMealPlanTool.create,
 ...
 * shared `TurnContext` data so a mid-turn `household__add_members` flows the new roster to tools built
