...
describe("classify → persist match → ingredient affinity (offline)", () => {
  ...
  it("penalizes the recipe when okra is disliked, rewards it when liked", async () => {
    ...
    expect(aff.score(rankable, prefs([{ facet: "ingredient", value: OKRA_BASE_ID, sentiment: "dislike" }]))).toBe(0);
    expect(aff.score(rankable, prefs([{ facet: "ingredient", value: OKRA_BASE_ID, sentiment: "like" }]))).toBe(1);
    // A stale/unknown base id matches nothing → fails safe (neutral).
    expect(aff.score(rankable, prefs([{ facet: "ingredient", value: "gone-uuid", sentiment: "dislike" }]))).toBe(0.5);
  });
});
